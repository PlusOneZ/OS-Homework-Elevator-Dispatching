  ...
  <div class="py-3">
    ...
    <div class="relative max-w-xl mx-auto text-center">
      <div class="relative shadow-lg sm:rounded-2xl bg-pink-50 rounded-xl sm:p-5 p-2">
        <div
            class="inline-flex pr-6 sm:p-3 rounded-3xl p-2"
            :class="disabled ? 'bg-gray-400' : 'bg-green-800'"
        >
        ...
        <div class="grid gap-4 grid-cols-4 pt-5">
          ...
          <div class="w-10">
            <button class="hover:shadow-lg" id="emergency" @click="emergencyCall(elevName)">
              <svg class="h-7 rounded border-2 border-solid border-red-300 hover:bg-red-800 w-8 "
                   :class="disabled ? 'bg-red-800' : 'bg-red-400'"
                ...
                <path
                    ...
                    :fill="disabled ? '#2f90b9' : '#f4ea2a'"></path>
                <path
...
export default {
  ...
  methods: {
    ...
    emergencyCall: function (name) {
      if (this.disabled) {
        alert(name + " emergency dealt, elevator now can work!")
        this.disabled = false
      } else {
        this.disabled = true
        alert(name + " emergency calls the console\nElevator is now stopping!")
        for (let i = 1; i <= this.floorNum; i++) {
          this.$set(this.lighten, i, false)
          if (this.upRequest[i]) {
            this.$emit("upRequest", i)
            this.upRequest[i] = false
          }
          if (this.downRequest[i]) {
            this.$emit("downRequest", i)
            this.downRequest[i] = false
          }
        }
        this.queue = []
